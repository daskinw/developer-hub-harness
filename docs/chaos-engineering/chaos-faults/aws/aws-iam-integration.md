...
1. [Enable the experiment service account to access AWS resources](aws-iam-integration.md#enable-the-experiment-service-account-to-access-aws-resources). This enables the Kubernetes service account used to control experiments to access resources across multiple AWS target accounts.
2. [Set up your target AWS accounts for IRSA](aws-iam-integration.md#set-up-your-target-accounts-for-irsa). The target accounts are the ones where you will run your chaos experiments.
3. [Establish a trust relationship](aws-iam-integration.md#establish-trust-between-the-aws-source-account-and-target-accounts) between the AWS source account and target accounts.
4. [Remove any references to secrets](aws-iam-integration.md#remove-all-secret-references-from-experiment-definitions) in chaos experiments.
...
* By default, chaos infrastructure associated with your chaos experiment has `litmus-admin` as a chaos service account. Based on the chaos experiment you wish to execute, you can configure the policy and attach the policy to a role, and go to [step 2](aws-iam-integration.md#associate-the-iam-role-with-the-experiment-service-account). Refer to [AWS superset policy](security-configurations/policy-for-all-aws-faults.md) to know more.
...
You can find the JSON for the trust relationship in **AWS IAM > **_**ROLE\_NAME**_** > Trust relationship** tab.
...
2. **In each target account:** Edit the trust relationship for the IAM role you [created on the target account](aws-iam-integration.md#step-1-create-an-iam-role-and-policy-in-each-aws-target-account) as shown in the example below.
