...
:::info note In the CI or Build stage type, container step is named Run, and it has the same functionality. :::
...
8.  Configure the step as follows:
    ...
    :::info note Depending upon our operation, we might have to adjust the memory limit of the container. If required, you can change Limit Memory from `500Mi` to `4000Mi`. :::
...
Variables such as project name and GitHub repository are runtime inputs. They are needed at the time of pipeline execution. When creating a new variable, you can specify its type in the UI. For more information about reference variables, go to the [reference documentation](../../platform/variables-and-expressions/harness-variables/) on pipeline variables.
...
![](static/template-1.png) ![](static/template-2.png) ![](static/template-3.png)
...
Use the URL to the `template.yaml` created above and register it by using the same process for [registering a new software component](../get-started/register-a-new-software-component/).
