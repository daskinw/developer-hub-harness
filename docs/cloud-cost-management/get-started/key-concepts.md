...
For GCP Kubernetes persistent volumes (\`PV\_TYPE\_GCE\_PERSISTENT\_DISK\`\`), the exact price rate is calculated by calling the GCP Kubernetes APIs. For all other cloud providers, a hardcoded price value is used for cost calculation.
