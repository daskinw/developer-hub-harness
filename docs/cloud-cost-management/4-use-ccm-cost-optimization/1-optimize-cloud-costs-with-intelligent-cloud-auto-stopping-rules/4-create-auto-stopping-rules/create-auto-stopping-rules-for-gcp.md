...
:::note You can use the same proxy load balancer for more than one rule managing resources (VMs, ASG) within the same VPC. :::
...
For more information on setting up access for Kubernetes clusters, see [Create AutoStopping Rules for a Kubernetes Cluster](create-autostopping-rules-for-kubernetes.md).
