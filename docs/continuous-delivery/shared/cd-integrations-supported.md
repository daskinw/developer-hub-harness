...
* **Overview:**
  * [Kubernetes](../deploy-srv-diff-platforms/kubernetes/kubernetes-deployments-overview/)
  * [Helm charts](../deploy-srv-diff-platforms/helm/helm-cd-quickstart/) (v2 and v3)
  * [Kustomize](../deploy-srv-diff-platforms/kustomize/kustomize-quickstart/)
  * Local ([Harness Community Edition](../deploy-srv-diff-platforms/community-ed/harness-community-edition-overview/))
...
* **Limitations:**
  * Helm:
    ...
    * Kustomize:
      ...
      * Kustomize manifests and patches do **not** support the [custom remote manifest](../deploy-srv-diff-platforms/kubernetes/cd-kubernetes-category/add-a-custom-remote-script-and-manifests/) feature.
...
For details on what you can deploy, go to [What Can I Deploy in Kubernetes?](../deploy-srv-diff-platforms/kubernetes/cd-k8s-ref/what-can-i-deploy-in-kubernetes/).
...
For details on other tools and versions included in Harness, see [Delegate-required SDKs](../../platform/delegates/delegate-reference/delegate-required-sdks/).
...
* **Overview:**
  * [Native Helm](../deploy-srv-diff-platforms/helm/native-helm-quickstart/)
* **Supported connectors for deployment:**
  * Kubernetes Connector
    * Username + Password
    * Client Key and Secret
...
* **Overview:**
  * [AWS ECS](../deploy-srv-diff-platforms/aws/ecs/ecs-deployment-tutorial/)
* **Supported connectors for deployment:**
* AWS Cloud Connector
  * IRSA
  * Access Key and Secret Key
...
* **Overview:**
  * [AWS AMI/ASG](../deploy-srv-diff-platforms/aws/asg-tutorial/)
* **Supported connectors for deployment:**
  * AWS cloud connector
    * IRSA
    * Access Key and Secret Key
...
* **Overview:**
  * [AWS Lambda](../deploy-srv-diff-platforms/aws/aws-lambda-deployments/)
* **Supported connectors for deployment:**
  * AWS Cloud Connector
    * IRSA
    * Access Key and Secret Key
...
* **Overview:**
  * [AWS SAM](../deploy-srv-diff-platforms/aws/aws-sam-deployments/)
* **Supported connectors for deployment:**
  * AWS cloud connector
    * Access key and secret key
* **Supported platforms for deployment:**
...
* **Overview:**
  * [WinRM](../deploy-srv-diff-platforms/traditional/win-rm-tutorial/)
* **Supported connectors for deployment:**
  * AWS Cloud Connector
    * IRSA
    * Access Key and Secret Key
...
* **Overview:**
  * [SSH](../deploy-srv-diff-platforms/traditional/ssh-ng/)
* **Supported connectors for deployment:**
  * AWS Cloud Connector
    * IRSA
    * Access Key and Secret Key
...
* **Overview:**
  * [Tanzu Application Services](../deploy-srv-diff-platforms/tanzu/tanzu-app-services-quickstart/)
* **Supported connectors for deployment:**
  * Tanzu Connector
...
* **Overview:**
  * [Google Cloud Functions](../deploy-srv-diff-platforms/google-functions/)
* **Supported connectors for deployment:**
...
* **Overview:**
  * [Spot Elastigroup deployments](../deploy-srv-diff-platforms/aws/spot-deployment/)
* **Supported connectors for deployment:**
  * Spot Connector
    * AccountID + API Token
* **Supported platforms for deployment:**
...
* **Overview:**
  * [Serverless.com Framework](../deploy-srv-diff-platforms/serverless/serverless-lambda-cd-quickstart/) (AWS Lambda)
* **Supported connectors for deployment:**
  * AWS Cloud Connector
    * IRSA
    * Access Key and Secret Key
...
* **Overview:**
  * [Azure Web Apps](../deploy-srv-diff-platforms/azure/azure-web-apps-tutorial/)
* **Supported connectors for deployment:**
  * Azure Cloud Connector (AKS Authentication)
    * Subscription Id
    * Principal and Service Account
...
Continuous Integration (CI) can be performed in Harness using the module and [CI pipelines](../../continuous-integration/get-started/key-concepts/).
...
* 2.397
* **Overview:**
  * [Run Jenkins jobs in CD pipelines](../x-platform-cd-features/cd-steps/builds/run-jenkins-jobs-in-cd-pipelines/)
...
* **Overview:**
  * [GitOps](../gitops/get-started/harness-git-ops-basics/)
  * [GitOps Quickstart](../gitops/get-started/harness-cd-git-ops-quickstart/)
...
* **Overview:**
  * [Custom deployments using Deployment Templates](../deploy-srv-diff-platforms/custom-deployment-tutorial/)

</details>

Harness supports the following infrastructure provisioning tools:

* [Terraform](../cd-infrastructure/terraform-infra/terraform-provisioning-with-harness/)
* [Terragrunt](../cd-infrastructure/terragrunt-howtos/)
* [Azure ARM](../cd-infrastructure/azure-arm-provisioning/)
* [Azure Blueprint](../cd-infrastructure/azure-blueprint-provisioning/)
* [AWS CloudFormation](../cd-infrastructure/cloudformation-infra/cloud-formation-how-tos/)
* [Shell script](../cd-infrastructure/shell-script-provisioning/) (custom)

#### Terraform version support

Harness does not include Terraform on the Harness Delegate. You must install Terraform on the Delegate when using Terraform in Harness. For more information, go to [Terraform How-tos](../cd-infrastructure/terragrunt-howtos/).
...
:::info note Harness also supports Terraform Cloud and Enterprise. :::

* **Containerized steps:**
  * [Containerize step groups](../x-platform-cd-features/cd-steps/containerized-steps/containerized-step-groups/)
  * Multiple step types can be run containerized.
* **Controls:**
  * [Strategies](../manage-deployments/deployment-concepts/): basic, rolling, canary, blue green, custom.
  * [Barriers](../manage-deployments/synchronize-deployments-using-barriers/)
  * [Resource Constraints](../manage-deployments/deployment-resource-constraints/)
  * [Queue steps](../manage-deployments/control-resource-usage-with-queue-steps/)
  * [Deployment freeze](../manage-deployments/deployment-freeze/)
  * [Failure strategies](../../platform/pipelines/failure-handling/define-a-failure-strategy-on-stages-and-steps/)
  * [Conditional executions](../../platform/pipelines/step-skip-condition-settings/)
  * [Looping strategies](../../platform/pipelines/looping-strategies/looping-strategies-matrix-repeat-and-parallelism/)
  * [Triggers](../../category/triggers/)
  * [Input set and overlays](../../platform/pipelines/input-sets/)
* **Utilities:**
  * [Run a Docker container in a CD stage](../x-platform-cd-features/cd-steps/utilities/container-step/)
  * [Using HTTP requests in CD pipelines](../x-platform-cd-features/cd-steps/utilities/http-step/)
  * [Using shell scripts in CD stages](../x-platform-cd-features/cd-steps/utilities/shell-script-step/)
  * [Use the Command step to download or copy artifacts and configs, or run scripts](../x-platform-cd-features/cd-steps/utilities/download-and-copy-artifacts-using-the-command-step/)
  * [Run a step on multiple target instances](../x-platform-cd-features/cd-steps/run-a-script-on-multiple-target-instances/)
  * [Wait step](../x-platform-cd-features/cd-steps/utilities/wait-step/)
  * [Email step](../x-platform-cd-features/cd-steps/utilities/email\_step/)
  * [JSON and XML functors](../x-platform-cd-features/cd-steps/utilities/json-and-xml-functors/)
* **Build:**
  * [Background step](../x-platform-cd-features/cd-steps/containerized-steps/background-step/)
  * [Git Clone step](../x-platform-cd-features/cd-steps/containerized-steps/git-clone-step/)
  * [Run step](../x-platform-cd-features/cd-steps/containerized-steps/run-step/)
  * [Plugin step](../x-platform-cd-features/cd-steps/containerized-steps/plugin-step/)

Manifests, specifications, config files, and other deployment files can be pulled from the following providers:

* [Harness File Store](../x-platform-cd-features/services/add-inline-manifests-using-file-store/)
* [Git on any platform](../../platform/connectors/code-repositories/ref-source-repo-provider/git-connector-settings-reference/)
* [Github](../../platform/connectors/code-repositories/ref-source-repo-provider/git-hub-connector-settings-reference/)
* [GitLab](../../platform/connectors/code-repositories/ref-source-repo-provider/git-lab-connector-settings-reference/)
* [Bitbucket](../../platform/connectors/code-repositories/ref-source-repo-provider/bitbucket-connector-settings-reference/)
* [AWS CodeCommit](../../platform/connectors/cloud-providers/ref-cloud-providers/aws-connector-settings-reference/)
* [Azure Repos](../../platform/connectors/code-repositories/connect-to-a-azure-repo/)
...
All artifact sources are covered in [CD artifact sources](../x-platform-cd-features/services/artifact-sources/).
...
* Jira ([ticketing](../x-platform-cd-features/cd-steps/ticketing-systems/create-jira-issues-in-cd-stages/), [approvals](../../platform/approvals/adding-jira-approval-stages/))
* ServiceNow ([ticketing](../x-platform-cd-features/cd-steps/ticketing-systems/create-service-now-tickets-in-cd-stages/), [approvals](../../platform/approvals/service-now-approvals/))
* [Harness manual approvals](../x-platform-cd-features/cd-steps/approvals/using-harness-approval-steps-in-cd-stages/)
...
* [Harness Policy As Code overview](../x-platform-cd-features/advanced/cd-governance/harness-governance-overview/)
* [Harness Policy As Code quickstart](../x-platform-cd-features/advanced/cd-governance/harness-governance-quickstart/)
* [Add a Policy step to a pipeline](../x-platform-cd-features/advanced/cd-governance/add-a-governance-policy-step-to-a-pipeline/)
