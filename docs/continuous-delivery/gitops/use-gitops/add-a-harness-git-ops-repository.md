...
description: >-
  This topic describes how to add a Harness GitOps repository containing the
  declarative description of a desired state.
---
...
A Harness GitOps Repository is used for Harness GitOps only. For other Harness features like CI, CD Pipelines, etc, use a standard [Git Connector](../../../platform/connectors/code-repositories/ref-source-repo-provider/git-connector-settings-reference/).

### Before you begin

If you are new to Harness GitOps, familiarize yourself with the following topics:

* [Harness GitOps Basics](../get-started/harness-git-ops-basics.md)
* [Harness CD GitOps Quickstart](../get-started/harness-cd-git-ops-quickstart.md)
...
6.  In **Credentials**, select one of the following:
    ...
    * **Specify Credentials for Repository**
      ...
      * For steps on setting up the GitHub App, go to [Use a GitHub App in a GitHub Connector](../../../platform/connectors/code-repositories/git-hub-app-support/).
...
8. In **Specify Repository Type**, select **Helm**.
9. In **Repository Name**, enter a name.
10. In **GitOps Agent**, select or create the Agent you want to use to fetch charts from this repo. See [Install a Harness GitOps Agent](install-a-harness-git-ops-agent.md).
11. In **Repository URL**, enter the URL to your HTTP Helm repository. For example, `https://charts.bitnami.com/bitnami`.
12. Select **Continue**.
13. In **Credentials**, select one of the following:
    * **Specify Credentials for Repository**
    ...
    * **Use a Credentials Template**
      *   Select the GitOps credentials template to use.
          ...
          Harness will auto-detect the repository credentials template (if any) based on the GitOps repository **URL** and auto-populate it. If Harness auto-populated the GitOps repository, then you cannot edit the repository credentials template setting.
14. Select **Save & Continue**. Harness validates the connection.
...
2. In **Specify Repository Type**, select **Helm**.
3. In **Repository Name**, enter a name.
4. In **GitOps Agent**, select or create the Agent you want to use to fetch charts from this repo. For details, go to [Install a Harness GitOps Agent](install-a-harness-git-ops-agent.md).
5. In **Repository URL**, enter the URL to your OCI Helm repository. For example, `us-east4-docker.pkg.dev/<gcp-project>/<repository>`.
6. Select **Enable OCI**
7. Select **Continue**.
8. In **Credentials**, select **Specify Credentials for Repository**.
