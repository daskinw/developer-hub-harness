...
description: >-
  Using Harness Git Experience with Feature Flags allows you to manage your
  Flags from a .yaml file in your Git repository.
---

# Manage flags using Git Experience

import git\_1 from './static/5-manage-featureflags-in-git-repos-04.png' import git\_2 from './static/5-manage-featureflags-in-git-repos-05.png' import git\_3 from './static/5-manage-featureflags-in-git-repos-06.png' import git\_4 from './static/5-manage-featureflags-in-git-repos-07.png' import git\_5 from './static/5-manage-featureflags-in-git-repos-08.png' import git\_6 from './static/4-git-blue-circle.png'\
import git\_7 from './static/5-manage-featureflags-in-git-repos-09.png' import git\_8 from './static/8-git-off.png'

:::warning There is a known issue with this feature. When you turn on a Feature Flag, some target rules may be reordered in your Git repo. This doesn't affect the functionality of your Feature Flag or targets and we are working to fix this issue as soon as possible. :::
...
:::info note This feature is not supported on the Harness Self-Managed Enterprise Edition (on-prem). :::
...
:::warning Syncing changes between a remote file and the Harness Platform can take up to 5 mins. During this window the changes are commited to the remote file but not yet pulled and synced by the Harness Platform. Any changes made to the Harness Platform within that window trigger remote file updates, which overwrite the content of the remote file. :::

### Prerequisites

#### Create at least one flag in Harness

In order to access the Git Sync setup in the Harness UI, you must [add at least one flag](ff-creating-flag/create-a-feature-flag/) in Harness.
...
:::warning Do not use **Git Management** in Project Setup. This is an older version of Git Experience that does not work with Feature Flags. :::

During the setup, you are asked to either select an existing Harness Git connector, or create a new one. If creating a new one, have your [PAT](manage-featureflags-in-git-repos.md#create-a-personal-access-token-if-adding-a-new-git-connector) ready to enter into the connector configuration. You can also create a Git connector before starting this procedure. Go to [Connect to a Git repository](../platform/connectors/code-repositories/connect-to-code-repo/) for instructions.
...
:::info note You must [add at least one flag](ff-creating-flag/create-a-feature-flag/) on this page in order to see the **Set Up Git Sync** button. :::
...
1. Configure these fields, and then select **Save**:
   *   **Git Connector** - Select an existing connector, or select **+ New Connector** to create a new one.
       ...
       If creating a new connector, you must enter the access token ([PAT](manage-featureflags-in-git-repos.md#create-a-personal-access-token-if-adding-a-new-git-connector)) to your Git repo in the Credentials step, and be sure to select **Enable API access**.

       ![New Git Connector form with Enable API access checkbox selected](static/gitex-connector-enable-api-access.png)

       For more information, go to [Connect to a Git repository](../platform/connectors/code-repositories/connect-to-code-repo/), and [Git connector settings reference](../platform/connectors/code-repositories/ref-source-repo-provider/git-connector-settings-reference/).
   * **Repository** - Select or enter the name of your repository.
   * **Git branch** - Select or enter a branch name.
   * **YAML Path** - Enter the path to the [YAML file](manage-featureflags-in-git-repos.md#example-yaml-file-for-flags) containing your flag information.
...
:::warning Turning sync on triggers an immediate attempt to sync Harness content to the remote file. Any changes you made to the remote file before syncing are overwritten. This can result in losing content or configurations that are not yet synced to Harness. :::

To turn on synchronization between the Harness Platform and Git:

1. Ensure that Git Experience has been [enabled](../platform/git-experience/configure-git-experience-for-harness-entities/) for your project.
...
1.  Select the branch, and then toggle **Sync with Git** on.
    ...
    (See [Auto-commit to the selected branch](manage-featureflags-in-git-repos.md#auto-commit-to-the-selected-branch) for the second toggle.)
...
This toggles on the [**Auto-commit to the selected branch**](manage-featureflags-in-git-repos.md#auto-commit-to-the-selected-branch) option, described below.
...
When you turn off syncing with Git, any changes you make to flags in Harness are not committed to the flags YAML file in your Git repository. Any changes to that file in your repository are not synced with Harness until you [turn on syncing](manage-featureflags-in-git-repos.md#turn-on-syncing-with-git) again.
...
This procedure removes the connection to the Git repository that you configured when you [set up Git Experience](manage-featureflags-in-git-repos.md#set-up-git-experience).
