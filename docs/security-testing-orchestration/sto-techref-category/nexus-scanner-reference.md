...
import security\_step\_nexus from './static/security-step-nexus.png'

![Configuring a Nexus scan in a Security step](%7Bsecurity\_step\_nexus%7D)
...
* [`scan_type`](security-step-settings-reference/#scanner-categories) = `repository`
* [`policy_type`](security-step-settings-reference/#data-ingestion-methods) = `orchestratedScan` or `dataLoad`
* When [`policy_type`](security-step-settings-reference/#data-ingestion-methods) is set to `orchestratedScan`:
...
* `fail_on_severity` - See [Fail on Severity](nexus-scanner-reference.md#fail-on-severity).
