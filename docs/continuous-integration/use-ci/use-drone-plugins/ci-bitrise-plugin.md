...
* `with:` If required by the Integration, provide a mapping of key-value pairs representing Integration inputs, such as `build_type: 'apk'`. For more information, go to [Settings](ci-bitrise-plugin.md#settings).
* `env:` If required by the Integration, provide a mapping of environment variables to pass to the Integration. For more information, go to [Environment Variables](ci-bitrise-plugin.md#environment-variables).

:::tip

If you already configured Bitrise Integrations elsewhere, you can [transfer Bitrise Integrations into Harness CI](ci-bitrise-plugin.md#transfer-bitrise-integrations-into-harness-ci).

You can use [expressions](../../../platform/variables-and-expressions/runtime-inputs/) in the `with` and `env` settings. For example, `credentials: <+stage.variables.[TOKEN_SECRET]>` uses an expression referencing a [stage variable](../../../platform/pipelines/add-a-stage/#stage-variables).
...
* Settings keys can be supplied as fixed values or runtime input, and values can be supplied as fixed values, runtime input, or expressions. For more information, go to [Fixed values, runtime inputs, and expressions](../../../platform/variables-and-expressions/runtime-inputs/).
...
If required by the Integration, add key-value pairs representing environment variables that you want to pass to the Integration, such as `GITHUB_TOKEN: <+secrets.getValue("github_pat")>`. Note that these are _incoming_ environment variables that you're passing to the Bitrise Integration, which are separate from _outgoing_ environment variables that are output by the Integration.Refer to the Integration's usage specifications for details about specific environment variables relevant to the Integration that you want to use.

:::tip

* You can use [fixed values, runtime inputs, or expressions](../../../platform/variables-and-expressions/runtime-inputs/) for environment variable values. For example, `<+stage.variables.[TOKEN_SECRET]>` is a variable expression [stage variable](../../../platform/pipelines/add-a-stage/#stage-variables).
...
| <pre><code>    inputs:
    - variant: debug
    - build_type: apk
</code></pre> | <pre class="language-yaml"><code class="lang-yaml">      with:
        variant: 'debug'
        build_type: 'apk'
</code></pre> |
