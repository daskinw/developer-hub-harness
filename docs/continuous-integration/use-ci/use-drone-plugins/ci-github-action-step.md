...
* [Private Action repos](ci-github-action-step.md#private-action-repositories)
* [Duplicate Actions](ci-github-action-step.md#duplicate-actions)
* [Actions requiring a defined working directory](ci-github-action-step.md#actions-requiring-a-defined-working-directory)

:::tip Tips

* If you already configured GitHub Actions elsewhere, you can quickly [transfer GitHub Actions into Harness CI](ci-github-action-step.md#transfer-github-actions-into-harness-ci) by copying the `spec` details from your existing GitHub Actions YAML.
* You can use variable expressions in the `with` and `env` settings. For example, `credentials: <+stage.variables.[TOKEN_SECRET]>` uses a [stage variable](../../../platform/pipelines/add-a-stage/#stage-variables) to call a token stored as a [Harness secret](../../../category/secrets/).
* For GitHub Actions steps, `with` mappings are automatically exported as [output variables](ci-github-action-step.md#output-variables-from-github-actions-steps).
...
6. Optionally, you can set the **Timeout**. Once the timeout limit is reached, the step fails and pipeline execution continues. To set skip conditions or failure handling for steps, go to:
   * [Step Skip Condition settings](../../../platform/pipelines/step-skip-condition-settings.md)
   * [Step Failure Strategy settings](../../../platform/pipelines/failure-handling/define-a-failure-strategy-on-stages-and-steps/)

:::tip Tips

* You can use fixed values, runtime inputs, or variable expressions for **Settings** and **Environment Variables** values. For example, `<+stage.variables.[TOKEN_SECRET]>` is a variable expression [stage variable](../../../platform/pipelines/add-a-stage/#stage-variables) that calls a token stored as a [Harness secret](../../../category/secrets/).
* For GitHub Actions steps, **Settings** are automatically exported as [output variables](ci-github-action-step.md#output-variables-from-github-actions-steps).
...
For more information about configuring the Action step's settings, go to the [Action step settings and specifications](ci-github-action-step.md#action-step-settings-and-specifications) section, above.
...
Some GitHub Actions need to run on the cloned [codebase](../codebase-configuration/create-and-configure-a-codebase/). The Action step doesn't automatically set a working directory.
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <pre class="language-yaml"><code class="lang-yaml">- name: hello-world
  uses: actions/hello-world-javascript-action@main
  with:
    who-to-greet: 'Mona the Octocat'
  env:
    hello: world
</code></pre> | <pre class="language-yaml"><code class="lang-yaml">- step:
    type: Action
    name: hello world
    identifier: hello_world
    spec:
