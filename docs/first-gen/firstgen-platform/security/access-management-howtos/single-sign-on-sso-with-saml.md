...
description: >-
  Covers Harness' Single Sign-On (SSO) integrations with Okta, Azure, Google G
  Suite, and OneLogin, via SAML.
---

# Single Sign-On (SSO) with SAML in FirstGen

This content is for [Harness FirstGen](../../../../continuous-delivery/get-started/upgrading/upgrade-nextgen-cd.md). Switch to [NextGen](../../../../platform/authentication/single-sign-on-saml.md).
...
Users are not created as part of the SAML SSO integration. Users are invited to Harness using their email addresses. Once they log into Harness, their email addresses are registered as Harness Users. For more information, go to [SAML SSO with Harness Overview](single-sign-on-sso-with-saml.md#saml\_sso\_with\_harness\_overview).The section describes the Okta-specific steps you must perform to use an Okta app for Harness SAML SSO:

* [Okta ​User Accounts​](single-sign-on-sso-with-saml.md#okta\_user\_accounts)
* [​SAML Endpoint URL​ for Okta](single-sign-on-sso-with-saml.md#endpoint\_url\_for\_okta)
* [Okta ​SAML Metadata File​](single-sign-on-sso-with-saml.md#okta\_saml\_metadata\_file)
* [​Enable and Test Okta SSO](single-sign-on-sso-with-saml.md#enable\_and\_test\_sso\_with\_okta)
* [SAML Authorization with Okta](single-sign-on-sso-with-saml.md#saml\_authorization\_with\_okta)
...
:::danger If you do not see a new tab, your browser might have blocked it with a pop-up blocker. Please disable the blocker temporarily and retry the TEST button. :::
...
:::danger If you become locked out of Harness because of an SSO issue, you must [contact Harness](mailto:support@harness.io) to have the SSO Provider disabled. :::
...
1.  Set up SAML SSO in Harness as described in [SAML SSO with Okta](single-sign-on-sso-with-saml.md#saml\_sso\_with\_okta).
...
5\. Enable Authorization in Harness. Now that you have assigned a group to your SAML app and added a Group Attribute Name, you can enable authorization in Harness. 1. In **Harness**, select **Continuous Security** > **Access Management** > **Authentication Settings**. 2. Click **SSO Providers**. 3. In **SSO Providers**, click the vertical ellipsis (**︙**) next to the SSO Provider you have set up for SSO authentication, and click **Edit**.
...
6\. Select **Add Entity ID** and enter your custom Entity ID. The default Entity ID is **app.harness.io**. The value you enter here will override the default Entity ID. 7. Click **SUBMIT**. The SAML SSO Provider is now set up to use the Group Attribute Name for authorization. 8. Link the SAML SSO Provider to the Harness User Group. We will create a new Harness User Group, but you can use an existing Group so long as your Harness User account is a member and that User account is registered using the same email address you used to register with your SAML provider.

:::important SSO should not be linked to the default **Account Administrator** group because this puts the group at risk of deletion. :::
...
Do not log out of Harness. Use a different Harness User account in a Chrome Incognito window to ensure that you can disable SSO if needed. 1. In Chrome, open an Incognito window, and navigate to Harness. 2. Log into Harness using the email address of a Harness User that is also used in the SAML provider group linked to the Harness User Group.
...
* [Azure User Accounts](single-sign-on-sso-with-saml.md#azure-user-accounts)
* [Endpoint URL for Azure](single-sign-on-sso-with-saml.md#endpoint-url-for-azure)
* [User Attributes Claims](single-sign-on-sso-with-saml.md#user-attributes-claims)
* [Azure SAML Metadata File](single-sign-on-sso-with-saml.md#azure-saml-metadata-file)
* [Enable and Test SSO with Azure](single-sign-on-sso-with-saml.md#enable-and-test-sso-with-azure)
* [SAML Authorization with Azure](single-sign-on-sso-with-saml.md#saml-authorization-with-azure)
...
6. Make sure to copy this secret and save it as an encrypted text secret. For detailed steps to create an encrypted text in Harness, go to [Use Encrypted text Secrets](../secrets-management/use-encrypted-text-secrets/).
...
:::info note You must set the above for both Delegated permissions and Application permissions. :::
...
Users are not created as part of the SAML SSO integration. Users are invited to Harness using their email addresses. Once they log into Harness, their email addresses are registered as Harness Users. For more information, go to [SAML SSO with Harness Overview](single-sign-on-sso-with-saml.md#saml\_sso\_with\_harness\_overview).
...
Google G Suite supports authentication, but not authorization. Users are not created as part of the SAML SSO integration. Users are invited to Harness using their email addresses. Once they log into Harness, their email addresses are registered as Harness Users. For more information, go to [SAML SSO with Harness Overview](single-sign-on-sso-with-saml.md#saml\_sso\_with\_harness\_overview).For information about G Suite and SAML, go to [Service provider SSO set up](https://support.google.com/a/answer/6349809?hl=en\&ref\_topic=6348126) from Google.
...
6. In **Step 3**, in **Application Name**, enter a name for the application. Make sure that the name does not include spaces or special characters.![](static/single-sign-on-sso-with-saml-59.png)
   1. Now you will get the Harness logo so your employees will easily identify Harness in their G Suite apps. In another browser tab, download the [Harness logo](https://github.com/wings-software/harness-docs/blob/main/harness\_logo.png) .
...
7. In **Step 4**, provide the Harness provider details.
   ...
   4. In **Entity ID**, enter **app.harness.io**.For Harness [On-Prem](../../../starthere-firstgen/harness-on-premise-versions.md), replace **app.harness.io** with your custom URL.\
      If you use a custom Harness subdomain in any Harness version, like **example.harness.io**, use that URL.
   5. In **Start URL**, enter the same URL you entered for **ACS URL**.
   6. Leave **Signed Response** unchecked, and click **NEXT**.
...
To set up OneLogin as a SAML SSO provider on Harness, you exchange the necessary information between the OneLogin Harness application and Harness. The following sections cover [Authentication](single-sign-on-sso-with-saml.md#onelogin\_authentication) steps, followed by [Authorization](single-sign-on-sso-with-saml.md#onelogin\_authorization) steps.
...
1. In OneLogin, select **Users** > **Users**.\
   If you prefer to assign _groups_ to roles, instead start at **Users** > **Groups**, and modify the following instructions accordingly.
...
4. In **Group Attribute Name**, type paste in the name of your the [Field Group](single-sign-on-sso-with-saml.md#group\_parameters) you configured in OneLogin.![](static/single-sign-on-sso-with-saml-82.png)
...
9. In **Group Name**, type or paste the name of the [Field Group](single-sign-on-sso-with-saml.md#group\_parameters) you configured in OneLogin.
...
After you've [synced](single-sign-on-sso-with-saml.md#sync\_groups) Users between OneLogin and Harness, users will be assigned to the designated Harness User Group upon your _next_ login to Harness. To test whether OneLogin authentication and authorization on Harness are fully functional:
...
Log in with the credentials you set up in OneLogin, and confirm that doing so logs you into your Harness account. 4. In Harness Manager, navigate to **Continuous Security** > **Access Management** > **User Groups**. 5. Click the Group you [synced](single-sign-on-sso-with-saml.md#sync\_groups) earlier. 6. Verify that users from the synced OneLogin group should now appear within this Harness User Group. If so, this confirms successful authorization.
...
* For **Okta**, please use the Okta Administrator **Classic UI** Dashboard to add an application and configure SAML settings. For more information, go to [Setting Up a SAML Application in Okta](https://developer.okta.com/standards/SAML/setting\_up\_a\_saml\_application\_in\_okta).
