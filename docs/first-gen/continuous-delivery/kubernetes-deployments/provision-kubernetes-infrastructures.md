...
* [Next Steps](provision-kubernetes-infrastructures.md#next\_steps)
...
![](static/provision-kubernetes-infrastructures-31.png) If Harness cannot pull the variables from your script, check your settings and try again. Ensure that your Source Repo Provisioner is working by clicking its **TEST** button.\
...
Typically, when you add an Environment, you specify the Infrastructure Definition for an _existing_ infrastructure. To use your Terraform Provisioner, you add the Terraform Provisioner to the Infrastructure Definition to identify a dynamically provisioned infrastructure _that will exist_.
...
:::note Harness supports first class Terraform Kubernetes provisioning for Google Kubernetes Engine (GKE). :::
...
:::note When you map a Terraform script output to a Harness field as part of a Service Mapping, the variable for the output, `${terrafrom.exact_name​}`, can be used anywhere in the Workflow that uses that Terraform Provisioner. :::
