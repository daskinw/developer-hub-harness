...
Workflows include an **Apply** step that allows you to deploy _any resource_ you have set up in the Service **Manifests** section.
...
The Apply step can deploy _all workload types_, including Jobs in any Workflow type.
...
If you need to show the logs _during_ job execution rather than after the Apply step, then modify the script and run the step in parallel with Apply.
