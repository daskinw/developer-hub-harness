...
For the **Delegate Selector** setting, see [Delegate Selector](create-a-kubernetes-blue-green-deployment.md#delegate\_selector) above.You can see that the primary Kubernetes service is represented by the variable `${k8s.primaryServiceName}`, and the stage service by the variable `${k8s.stageServiceName}`. You can see how the swap works in the **Swap Primary with Stage** step in Harness Deployments.
