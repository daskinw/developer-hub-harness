...
description: "For GitHub, GitLab, and Bitbucket, you can trigger Build Workflows or a Build and Deploy Pipeline in response to a Git event using Webhooks using a Harness\_On Webhook Event\_Trigger. For example, the…"
---
...
3. In **Event Type**, select the event type for the Webhook event.\
   There are different options depending on the repo selected in **Payload Type**. See [Review: Payload and Event Type Matrix](trigger-a-deployment-on-git-event.md#review\_payload\_and\_event\_type\_matrix) below.![](static/trigger-a-deployment-on-git-event-19.png)
...
| BitBucket                                                                                                                           | <p>On Pull Request<br>On Repository<br>On Issue</p>                        |
| GitHub                                                                                                                              | <p>On Pull Request<br>On Push<br>On Delete<br>On Release<br>On Package</p> |
| GitLab                                                                                                                              | <p>On Pull Request<br><br>On Push</p>                                      |
| <p>Custom or no selection.<br>This option is for repos other than the default Git providers.<br>For example, Bamboo or Jenkins.</p> | <p>On Pull Request<br>On Push</p>                                          |
