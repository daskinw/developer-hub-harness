...
In order to obtain the names of the host(s), pod(s), or container(s) where your service is deployed, the verification provider should be added to your workflow _after_ you have run at least one successful deployment.In this topic:
...
2. Locate an event using a search query. For more information, see [Event Stream](https://docs.datadoghq.com/dashboards/widgets/event\_stream/) from Datadog.
