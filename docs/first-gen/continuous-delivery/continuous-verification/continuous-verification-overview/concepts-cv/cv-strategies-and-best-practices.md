...
description: >-
  Learn about analysis strategies, results tuning, and best practices for
...
You can use [Pin as Baseline for Continuous Verification](cv-strategies-and-best-practices.md#pin\_as\_baseline\_for\_continuous\_verification) to specify a deployment to use as a baseline.
...
* Don't use Previous Analysis in any phase of a _multiphase_ Canary deployment.
...
If you do not use Pin as Baseline, Harness uses default criteria. See [Baseline for Previous Analysis](cv-strategies-and-best-practices.md#baseline\_for\_previous\_analysis).By default in a Previous Analysis strategy, Harness uses the Continuous Verification data of the last successful Workflow execution **with data** as the baseline for the current analysis. This is an automatic setting, but you can select a specific deployment as a new baseline.
