...
* [Before You Begin](ami-blue-green.md#before\_you\_begin)
* [Overview](ami-blue-green.md#overview)
* [Limitations](ami-blue-green.md#limitations)
* [Blue/Green with Incremental Traffic Shift Summary](ami-blue-green.md#blue\_green\_with\_incremental\_traffic\_shift\_summary)
* [Blue/Green with Instant Traffic Shift Summary](ami-blue-green.md#blue\_green\_with\_instant\_traffic\_shift\_summary)
* [Prerequisites](ami-blue-green.md#prerequisites)
* [AWS Setup (Example)](ami-blue-green.md#aws\_setup\_example)
* [Define the Blue/Green Infrastructure](ami-blue-green.md#define\_the\_blue\_green\_infrastructure)
* [Infrastructure Provisioners](ami-blue-green.md#infrastructure\_provisioners)
* [Blue/Green with Incremental Traffic Shift](ami-blue-green.md#blue\_green\_with\_incremental\_traffic\_shift)
* [Blue/Green with Instant Traffic Shift](ami-blue-green.md#blue\_green\_with\_instant\_traffic\_shift)
* [Rollbacks and Downsizing Old ASGs](ami-blue-green.md#rollbacks\_and\_downsizing\_old\_as\_gs)
* [Support for Scheduled Scaling](ami-blue-green.md#support\_for\_scheduled\_scaling)
* [Troubleshooting](ami-blue-green.md#troubleshooting)
...
An AMI Blue/Green deployment requires you to set up the following resources up within AWS (example setup [below](ami-blue-green.md#aws\_setup\_bg)):
...
* [Launch Configuration and Launch Template Support](ami-blue-green.md#launch\_configuration\_and\_launch\_template\_support)
* [Launch Configuration](ami-blue-green.md#launch\_configuration)
* [Auto Scaling Group](ami-blue-green.md#auto\_scaling\_group)
* [Target Groups](ami-blue-green.md#target\_groups)
* [Application Load Balancer (ALB)](ami-blue-green.md#application\_load\_balancer\_alb)
...
The Auto Scaling Group that you define in AWS must use the base [Launch Configuration](ami-blue-green.md#launch\_config\_bg) created above or a launch template. When you later select this ASG in your Harness Infrastructure Definition, it becomes the base ASG from which Harness will create new ASGs to deploy new AMIs.
...
Note that if you choose to instead configure scaling policies for your base ASG, Harness will apply these scaling policies in your Workflow's final  [Upgrade AutoScaling Group step](ami-blue-green.md#upgrade\_asg\_bg). (Harness will also oblige these policies in any rollback steps.)
...
3. In the resulting **Infrastructure Definition** dialog, enter a **Name** that will identify this Infrastructure Definition when you [add it to a Workflow](ami-blue-green.md#workflow\_bg).
...
7. Select the **Region** and base **Auto Scaling Group** that you [configured in AWS](ami-blue-green.md#asg\_bg) for Blue/Green.
8. If you want this Infrastructure Definition to create a new ASG numbering series based on each new selection in the above **Auto Scaling Groups** drop-down, enable the check box labeled **Reset ASG revision numbers each time a new base ASG is selected**. For details on this option, see [Reset ASG Revision Numbers](ami-blue-green.md#reset\_asg\_rev).
9. In the upper **Target Groups (for ALB)** field, select the Target Group that you [configured in AWS](ami-blue-green.md#target\_groups\_bg) as your production group.
...
You're now ready to create a Blue/Green deployment [Workflow](ami-blue-green.md#workflow\_bg).
...
If you instead select the **Map Dynamically Provisioned Infrastructure** option along with the **Reset ASG revision numbers...** option, Harness will automatically create a new numbering series based on the new value for the base ASG value that your  [Infrastructure Provisioner](ami-blue-green.md#infrastructure\_provisioners) outputs in a variable expression.
...
This section contains the steps for the deployment option described above in [Blue/Green with Incremental Traffic Shift Summary](ami-blue-green.md#blue\_green\_with\_incremental\_traffic\_shift\_summary).

**Harness Delegate, Service, and Infrastructure Definition Requirements**

There are no specific Harness Delegate, Service, and Infrastructure Definition requirements beyond the standard setup described in [Prerequisites](ami-blue-green.md#prerequisites).
...
This section contains the steps for the deployment option described above in  [Blue/Green with Instant Traffic Shift Summary](ami-blue-green.md#blue\_green\_with\_instant\_traffic\_shift\_summary).

By default, Harness AMI Blue/Green Workflows have five steps:

1. [Setup AutoScaling Group](ami-blue-green.md#setup\_asg\_bg): Specify how many instances to launch, their resizing order, and their steady state timeout.
2. [Deploy Service](ami-blue-green.md#upgrade\_asg\_bg): Specify the number or percentage of instances to deploy within the ASG you've configured in [Setup AutoScaling Group](ami-blue-green.md#setup\_asg\_bg).
3. Verify Staging: Optionally, specify Verification Providers or Collaboration Providers.
4. [Swap Routes](ami-blue-green.md#swap\_routes\_bg): Re-route requests to the newest stable version of your ASG.
...
5. Select the **Infrastructure Definition** you [configured earlier](ami-blue-green.md#svc\_infra\_bg) for AMI Blue/Green deployment. The dialog will now look something like this: ![](static/ami-blue-green-66.png)
...
Harness recommends setting the **Auto Scaling Steady State Timeout (mins)** field to at least **20** minutes, as shown above. This is a safe interval to prevent failed deployments while the [Swap Routes](ami-blue-green.md#swap\_routes\_bg) step's Blue/Green switchover completes.
...
If your base Auto Scaling Group is configured in AWS with [scaling policies](ami-blue-green.md#scaling\_policies), Harness will apply those policies in your Workflow's final **Upgrade AutoScaling Group** step.
...
Finally, the old ASG is downsized to 0 instances. Had the Workflow's [Step 4 (Swap Routes)](ami-blue-green.md#swap\_routes\_bg) _not_ specified **Downsize Old Auto Scaling Group,** these resources would not be explicitly freed up:
...
To resolve the rollbacks: In your Blue/Green Workflow's [Step 1](ami-blue-green.md#setup\_asg\_bg) (**Setup AutoScaling Group**), try raising the **Auto Scaling Steady State Timeout (mins)** setting to at least match the switchover interval you observe in the AWS Console.
