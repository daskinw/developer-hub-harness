...
In Terraform Destroy, you _disable_ the **Inherit from last successful Terraform Apply** option, and then provide the input value and remote state settings to use.
...
As described in [Review: What Gets Destroyed?](terraform-destroy.md#review\_what\_gets\_destroyed), select this option to destroy the infrastructure provisioned by the last successful **Terraform Provision** or **Terraform** **Apply** step in the Workflow.
